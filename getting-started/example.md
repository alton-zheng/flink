...
---
以下示例展示了 Flink 从简单的WordCount到图算法的应用。示例代码展示了 [Flink’s DataSet API](https://ci.apache.org/projects/flink/flink-docs-release-1.9/zh/dev/batch/index.html) 的使用。
...
在这个简单的示例中，PageRank 算法由一个[批量迭代](https://ci.apache.org/projects/flink/flink-docs-release-1.9/zh/dev/batch/iterations.html)和一些固定次数的迭代实现。
...
[PageRank代码](https://github.com/apache/flink/blob/master//flink-examples/flink-examples-batch/src/main/scala/org/apache/flink/examples/scala/graph/PageRankBasic.scala)实现了以上示例。 他需要以下参数来运行: --pages <path> --links <path> --output <path> --numPages <n> --iterations <n>。
...
这个代码实现使用了[增量迭代](https://ci.apache.org/projects/flink/flink-docs-release-1.9/zh/dev/batch/iterations.html)： 没有改变其组件 ID 的顶点不会参与下一轮迭代。这种方法会带来更好的性能，因为后面的迭代可以只处理少量的需要计算的顶点。
...
[ConnectedComponents代码](https://github.com/apache/flink/blob/master//flink-examples/flink-examples-batch/src/main/scala/org/apache/flink/examples/scala/graph/ConnectedComponents.scala) 实现了以上示例。他需要以下参数来运行: --vertices <path> --edges <path> --output <path> --iterations <n>。
