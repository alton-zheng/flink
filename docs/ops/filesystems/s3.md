...
[Amazon Simple Storage Service](http://aws.amazon.com/s3/) (Amazon S3) 提供用于多种场景的云对象存储。S3 可与 Flink 一起使用以读取、写入数据，并可与 [流的 **State backends**](../state/state_backends.md) 相结合使用。
...
注意这些例子并*不详尽*，S3 同样可以用在其他场景，包括 [JobManager 高可用配置](../jobmanager_high_availability.md) 或 [RocksDBStateBackend](../state/state_backends.md#the-rocksdbstatebackend)，以及所有 Flink 需要使用文件系统 URI 的位置。
...
Flink 提供两种文件系统用来与 S3 交互：`flink-s3-fs-presto` 和 `flink-s3-fs-hadoop`。两种实现都是独立的且没有依赖项，因此使用时无需将 Hadoop 添加至 classpath。
  ...
  文件系统可以使用与 [Hadoop S3A 完全相同的配置方法](https://hadoop.apache.org/docs/stable/hadoop-aws/tools/hadoop-aws/index.html#S3A)进行配置，即将配置添加到 `flink-conf.yaml` 文件中。它是唯一一个支持 [StreamingFileSink](../../dev/connectors/streamfile_sink.html) 的文件系统。
...
例如某作业使用了 [StreamingFileSink](../../dev/connectors/streamfile_sink.md)，它仅支持 Hadoop，但建立 checkpoint 使用 Presto。在这种情况下，建议明确地使用 *s3a://* 作为 sink (Hadoop) 的 scheme，checkpoint (Presto) 使用 *s3p://*。
