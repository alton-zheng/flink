...
- [SELECT (查询)](queries.md)
- [CREATE TABLE, DATABASE, FUNCTION](create.md)
- [DROP TABLE, DATABASE, FUNCTION](drop.md)
- [ALTER TABLE, DATABASE, FUNCTION](alter.md)
- [INSERT](insert.md)

## 数据类型

请参考专门描述该主题的页面 [数据类型](../../table/types.md)。

通用类型与（嵌套的）符合类型 （如：POJO、tuples、rows、Scala case 类) 都可以作为行的字段。

符合类型的字段任意的嵌套可被 [值访问函数](../../table/functions/systemFunctions.md#value-access-functions) 访问。

通用类型将会被视为一个黑箱，且可以被 [用户自定义函数](../../table/functions/udfs.md) 传递或引用。

对于 DDL 语句而言，我们支持所有在 [数据类型](../../table/types.md) 页面中定义的数据类型。
